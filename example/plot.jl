...
function linearscale(
    ...
    ylim::Tuple{Float64,Float64} = (-0.5, 0.5),
)
...
function plansubplot(
    subplotsize::Tuple{Int,Int} = (8, 6),
    xrange::Tuple{Float64,Float64} = (0.1, 0.95),
    xsplit::Float64 = 0.05,
    ysplit::Float64 = 0.01,
    proportion::Vector{Float64} = ones(6),
)
    @assert subplotsize[2] == length(proportion)
    frameinfo =
        Matrix{Tuple{Tuple{Float64,Float64},Tuple{Float64,Float64}}}(undef, subplotsize)
...
heatmap!(
    ...
    range(; start = -1.0, stop = 1.0, length = 1001),
    SeisTools.Source.beachball_bitmap(
        SeisTools.Source.MomentTensor(mech[1], mech[2], mech[3]);
...
l = SeisTools.Source.beachball_sdrline(
    SeisTools.Source.MomentTensor(mech[1], mech[2], mech[3]),
)
